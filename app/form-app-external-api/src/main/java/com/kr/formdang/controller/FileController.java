    ...
    public ResponseEntity<RootResponse> uploadFileProfile(@ModelAttribute @Valid FileProfileRequest fileRequest, @RequestHeader("Authorization") String token) {
        try {
            ...
            if (result) { // 프로필 등록 성공
                ...
                JwtTokenResponse jwtTokenResponse = tokenService.getLoginToken(String.valueOf(authUser.getId()), authUser.getName(), profile.getPath(), accessKey); // 폼당폼당 JWT 토큰 요청 (프로필 내용 변경)
